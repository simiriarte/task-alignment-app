...
export default class extends Controller {
  connect() {
    console.log('Filter tasks drop zone connected')
  ...
  dragEnter(event) {
    event.preventDefault()
    console.log('Drag enter on filter tasks section')
  ...
  async drop(event) {
    event.preventDefault()
    console.log('Drop event on Filter Tasks section')
    ...
    try {
      ...
      if (dragData.type === 'subtask') {
        await this.convertSubtaskToMainTask(dragData)
      }
    } catch (error) {
      console.error('Error processing drop:', error)
      this.showError('Failed to convert subtask to main task')
    }
  }

  async convertSubtaskToMainTask(dragData) {
    try {
      ...
      if (response.ok && data.success) {
        console.log('Subtask converted successfully:', data)
        ...
        // Add the new task to the Filter Tasks section
        if (data.task_html) {
        ...
        this.showSuccess(`Converted "${dragData.title}" to main task`)
